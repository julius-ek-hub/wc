...
function NewUser() {
    ...
    const handleSubmit = async () => {
        ...
        socket.emit('new-account', info, response => {
            resetSettings({ ...response.account, RTC_APPID: response.RTC_APPID });
        ...
        <Center width="100%" >
            <Stack rowGap={1} width={lg ? '500px' : (md ? '60%' : '80%')}>
                ...
                <TextField
                    ...
                    helperText={error.telephone ? error.telephone : "*Required, You can use any number. There is no verification"}
                />
                <TextField
                    label="Email"
                    value={info.email}
